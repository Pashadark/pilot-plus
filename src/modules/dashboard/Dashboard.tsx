...
export function Dashboard({ stats, events, vehicles }: DashboardProps) {
  return (
    <div className="grid gap-6 lg:gap-8">
      ...
      <section aria-label="Показатели автопарка" className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => <FleetStatCard key={stat.id} stat={stat} />)}
      </section>
