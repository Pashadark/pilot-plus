...
import type { OperationActionState, WashStatus } from './types';
...
const INVALID_TRANSITION_ERROR: OperationActionState = {
  status: 'error',
  message: 'Недопустимый переход статуса мойки.',
};
...
const statuses: readonly WashStatus[] = ['PLANNED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'];
...
function parseStatus(value: string): WashStatus | null {
...
function transitionData(targetStatus: WashStatus): Prisma.WashRecordUpdateManyMutationInput {
  if (targetStatus === 'IN_PROGRESS') {
    return { status: targetStatus, startedAt: new Date() };
  }
...
export async function createWashAction(
  ...
  const input = parseWashInput(formData);
  ...
  revalidatePath('/wash');
  return { status: 'success', message: 'Мойка запланирована.' };
}

export async function transitionWashAction(
  _previousState: OperationActionState,
  formData: FormData,
): Promise<OperationActionState> {
  const session = await getAuthenticatedSession();
  ...
  if (!recordId || !fromStatus || !toStatus || !canTransitionWash(fromStatus, toStatus)) {
    return INVALID_TRANSITION_ERROR;
  }

  try {
    const updated = await prisma.washRecord.updateMany({
      where: {
        id: recordId,
        status: fromStatus,
  ...
  revalidatePath('/wash');
  return { status: 'success', message: 'Статус мойки обновлён.' };
}
